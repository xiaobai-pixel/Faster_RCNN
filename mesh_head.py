...
class MeshRCNNGraphConvHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: ShapeSpec):
        ...
        # fmt: off
        num_stages         = cfg.MODEL.ROI_MESH_HEAD.NUM_STAGES #1
        num_graph_convs    = cfg.MODEL.ROI_MESH_HEAD.NUM_GRAPH_CONVS  # per stage 1
        graph_conv_dim     = cfg.MODEL.ROI_MESH_HEAD.GRAPH_CONV_DIM #256
        graph_conv_init    = cfg.MODEL.ROI_MESH_HEAD.GRAPH_CONV_INIT #"normal"
        input_channels     = input_shape.channels
        # fmt: on

        self.stages = nn.ModuleList()
        for i in range(num_stages):
            vert_feat_dim = 0 if i == 0 else graph_conv_dim #256
